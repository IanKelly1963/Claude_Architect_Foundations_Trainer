...
*Version 1.0 · Effective July 2026 · Exam code: CCAR-F · This guide is subject to change without notice.*
...
- Configuring and customising Claude Code for team workflows using CLAUDE.md files, Agent Skills, MCP server integrations, and plan mode
...
| Delivery | Proctored: online proctored and/or test centre, per programme policy |
...
Skills in:
- Designing coordinator agents that analyse query requirements and dynamically select which subagents to invoke rather than always routing through the full pipeline
- Partitioning research scope across subagents to minimise duplication (e.g., assigning distinct subtopics or source types to each agent)
...
- Decomposing multi-concern customer requests into distinct items, then investigating each in parallel using shared context before synthesising a unified resolution
...
**Task Statement 1.5: Apply Agent SDK hooks for tool call interception and data normalisation**
...
- Prompt chaining patterns that break reviews into sequential steps (e.g., analyse each file individually, then run a cross-file integration pass)
...
- Decomposing open-ended tasks (e.g., "add comprehensive tests to a legacy codebase") by first mapping structure, identifying high-impact areas, then creating a prioritised plan that adapts as dependencies are discovered
...
- The importance of informing the agent about changes to previously analysed files when resuming sessions after code modifications
...
- Why agents with tools outside their specialisation tend to misuse them (e.g., a synthesis agent attempting web searches)
...
Skills in:
- Restricting each subagent's tool set to those relevant to its role, preventing cross-specialisation misuse
...
- MCP resources as a mechanism for exposing content catalogues (e.g., issue summaries, documentation hierarchies, database schemas) to reduce exploratory tool calls
...
- Exposing content catalogues as MCP resources to give agents visibility into available data without requiring exploratory tool calls
...
**Task Statement 3.1: Configure CLAUDE.md files with appropriate hierarchy, scoping, and modular organisation**
...
- .claude/rules/ directory for organising topic-specific rule files as an alternative to a monolithic CLAUDE.md
...
- Using the /memory command to verify which memory files are loaded and diagnose inconsistent behaviour across sessions
...
- Personal skill customisation: creating personal variants in ~/.claude/skills/ with different names to avoid affecting teammates
...
- Writing test suites covering expected behaviour, edge cases, and performance requirements before implementation, then iterating by sharing test failures
...
Knowledge of:
- The importance of explicit criteria over vague instructions (e.g., "flag comments only when claimed behaviour contradicts actual code behaviour" vs "check that comments are accurate")
...
- How few-shot examples enable the model to generalise judgement to novel patterns rather than matching only pre-specified cases
...
- Providing few-shot examples distinguishing acceptable code patterns from genuine issues to reduce false positives while enabling generalisation
...
- Adding enum values like "unclear" for ambiguous cases and "other" + detail fields for extensible categorisation
- Including format normalisation rules in prompts alongside strict output schemas to handle inconsistent source formatting
...
- Using prompt refinement on a sample set before batch-processing large volumes to maximise first-pass success rates and reduce iterative resubmission costs
...
Knowledge of:
- Progressive summarisation risks: condensing numerical values, percentages, dates, and customer-stated expectations into vague summaries
...
Skills in:
- Extracting transactional facts (amounts, dates, order numbers, statuses) into a persistent "case facts" block included in each prompt, outside summarised history
...
- Placing key findings summaries at the beginning of aggregated inputs and organising detailed results with explicit section headers to mitigate position effects
...
- Summarising key findings from one exploration phase before spawning sub-agents for the next phase, injecting summaries into initial context
...
- Having models output field-level confidence scores, then calibrating review thresholds using labelled validation sets
- Routing extractions with low model confidence or ambiguous/contradictory source documents to human review, prioritising limited reviewer capacity

**Task Statement 5.6: Preserve information provenance and handle uncertainty in multi-source synthesis**

Knowledge of:
- How source attribution is lost during summarisation steps when findings are compressed without preserving claim-source mappings
...
- Structuring reports with explicit sections distinguishing well-established findings from contested ones, preserving original source characterisations and methodological context
...
- Implement a human review routing strategy: have the model output field-level confidence scores, route low-confidence extractions to human review, and analyse accuracy by document type and field to verify consistent performance.
